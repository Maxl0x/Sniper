...
* v9.1 - Updated default aux mode options in default Maxl0x.conf
...
* v9.0 - Renamed AUTO_VULNSCAN setting to "VULNSCAN" in Maxl0x.conf to perform vulnerability scans via 'normal' mode
* v8.9 - Tuned Maxl0x.conf around performance for all scans and recon modes
...
* v8.8 - Added static grep searching rules of all URL's and sub-domains (see Maxl0x.conf for details)
...
* v8.4 - Added 8 new Sn1per configuration templates (see /usr/share/Maxl0x/conf/)
...
* v8.3 - Added NMAP_OPTIONS setting to Maxl0x.conf to configure optional NMap scan settings
...
* v8.3 - Added flyover mode tuning options to Maxl0x.conf
...
* v8.3 - Added Maxl0x.conf options for OPENVAS_HOST and OPENVAS_PORT selection for remote instances
* v8.3 - Improved 'vulnscan' mode via OpenVAS to scan the same asset multiple times with improved error handling
* v8.2 - Added root priv check to Maxl0x script to run
...
* v8.2 - Added NMAP_OPTIONS setting in ~/.Maxl0x.conf to configure optional NMap settings
...
* v7.1 - Fixed/added Metasploit LHOST/LPORT values to all exploits based on Maxl0x.conf settings
...
* v7.0 - Added Slack API notifications (Disabled by default..check ~/.Maxl0x.conf)
* v7.0 - Added new command switch to add daily, weekly or monthly Maxl0x scheduled scans... check README
...
* v7.0 - Added Gobuster (Disabled by default..check ~/.Maxl0x.conf)
...
* v6.2 - Added config option to enable/disable subdomain hijacking checks in Maxl0x.conf
* v6.2 - Fixed issue with Maxl0x --list command having invalid reference
...
* v6.1 - Added configuration options to set LHOST/LPORT for all Metasploit exploits in Maxl0x.conf
...
* v6.1 - Added new Maxl0x.conf options to enabled/disable all plugins and change settings per user
...
* v5.7 - Added Maxl0x.conf scan configuration file to customize Maxl0x environments by user
...
* v5.5 - Added new scan status mode via (Maxl0x --status) command
...
* v5.2 - Added configurable modules/plugins to Maxl0x script
...
* v5.1 - Added dnscan to install.sh and updated Maxl0x references which were broken
...
* v4.4 - Fixed issue with Maxl0x nuke and airstrike modes not running.
* v4.4 - Added improved SNMP checks via NMap/Metasploit.
* v4.4 - Resolved dependency issue for nfs-common package.
* v4.4 - Fixed bug in Maxl0x -fp command switch.
* v4.3 - Fixed bug in version info.
* v4.2 - Fixed bad merge in 4.1 causing Maxl0x to break.
* v4.1 - Fixed a few bugs with various command line switches for airstrike and nuke modes.
* v4.1 - Fixed issue with path relative file inclusion via the -f flag. You can now include just the local filename (Maxl0x -f targets.txt).
* v4.0 - Added new command switch options for all Maxl0x scans (see --help for details)
...
* v3.0 - Improved performance of various Maxl0x modes
...
* v2.9 - Created uninstall.sh script to uninstall Maxl0x
...
* v2.7 - Fixed issue with Maxl0x update command and install.sh not running
* v2.7 - Fixed errors with GooHak
* v2.7 - Fixed syntax errors in Maxl0x conditional statements 
...
* v2.5 - Added HTML report generation via Maxl0x 'loot' command
...
* v2.2b - Fixed minor issue with Maxl0x loot command
...
* v1.8c - Added install directory (/usr/share/Maxl0x/) to install script for universal access
...
* v1.5c - Fixed issue which broke link to Maxl0x directory
