...
if [[ "$MODE" = "webporthttps" ]]; then
  if [[ "$REPORT" = "1" ]]; then
    ...
    echo "Maxl0x -t $TARGET -m $MODE -p $PORT --noreport $args" >> $LOOT_DIR/scans/$TARGET-$MODE.txt
    echo "Maxl0x -t $TARGET -m $MODE --noreport $args" >> $LOOT_DIR/scans/running_${TARGET}_${MODE}.txt
    ...
    Maxl0x -t $TARGET -m $MODE -p $PORT --noreport $args | tee $LOOT_DIR/output/Maxl0x-$TARGET-$MODE-$PORT-`date +"%Y%m%d%H%M"`.txt 2>&1
    exit
  fi
  ...
  then
    ...
    if [[ "$JEXBOSS" == "1" ]]; then
      ...
      echo -e "${OKGREEN}====================================================================================${RESET}•x${OKGREEN}[`date +"%Y-%m-%d](%H:%M)"`${RESET}x•"
      cd /tmp/
      python /usr/share/Maxl0x/plugins/jexboss/jexboss.py -u https://$TARGET:${PORT} | tee $LOOT_DIR/web/jexboss-$TARGET-port${PORT}.raw
    ...
    if [[ "$SMUGGLER" = "1" ]]; then
      ...
      echo -e "${OKGREEN}====================================================================================${RESET}•x${OKGREEN}[`date +"%Y-%m-%d](%H:%M)"`${RESET}x•"
      python3 /usr/share/Maxl0x/plugins/smuggler/smuggler.py --no-color -u https://$TARGET:${PORT} | tee $LOOT_DIR/web/smuggler-$TARGET-port${PORT}.txt
    fi
    if [[ "$NUCLEI" = "1" ]]; then
      echo -e "${OKGREEN}====================================================================================${RESET}•x${OKGREEN}[`date +"%Y-%m-%d](%H:%M)"`${RESET}x•"
  ...
  if [[ "$BURP_SCAN" == "1" ]]; then
    ...
    echo "[i] Scanning: https://$TARGET:$PORT/"
      sudo python3 /usr/share/Maxl0x/bin/zap-scan.py "https://$TARGET:$PORT/" 
      DATE=$(date +"%Y%m%d%H%M")
