...
if [[ "$MODE" = "vulnscan" ]]; then
  if [[ "$REPORT" = "1" ]]; then
    ...
    echo "Maxl0x -t $TARGET -m $MODE --noreport $args" >> $LOOT_DIR/scans/$TARGET-vulnscan.txt
  ...
  if [[ "$NESSUS" = "1" ]]; then
    ...
    echo -e "${OKGREEN}====================================================================================${RESET}•x${OKGREEN}[`date +"%Y-%m-%d](%H:%M)"`${RESET}x•"
    bash /usr/share/Maxl0x/bin/nessus.sh $TARGET $NESSUS_KEY $NESSUS_HOST $NESSUS_USERNAME $NESSUS_PASSWORD $NESSUS_POLICY_ID $LOOT_DIR
  fi
  if [[ "$OPENVAS" = "1" ]]; then
