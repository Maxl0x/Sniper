if [[ "$MODE" = "webscan" ]]; then
	...
	echo "Maxl0x -t $TARGET -m $MODE --noreport $args" >> $LOOT_DIR/scans/running_${TARGET}_${MODE}.txt 2> /dev/null
    ...
    if [[ "$ZAP_SCAN" == "1" ]]; then
    	...
    	sudo python3 /usr/share/Maxl0x/bin/zap-scan.py "http://$TARGET/" 
    	DATE=$(date +"%Y%m%d%H%M")
    	sudo grep "'" /usr/share/Maxl0x/bin/zap-report.txt | cut -d\' -f2 | cut -d\\ -f1 > $LOOT_DIR/web/zap-report-$TARGET-http-$DATE.html
    	...
    	sudo python3 /usr/share/Maxl0x/bin/zap-scan.py "https://$TARGET/"
    	sudo grep "'" /usr/share/Maxl0x/bin/zap-report.txt | cut -d\' -f2 | cut -d\\ -f1 > $LOOT_DIR/web/zap-report-$TARGET-https-$DATE.html
	...
	if [[ "$NUCLEI" = "1" ]]; then
		...
		echo -e "${OKGREEN}====================================================================================${RESET}•x${OKGREEN}[`date +"%Y-%m-%d](%H:%M)"`${RESET}x•"
		nuclei -silent -t /usr/share/Maxl0x/plugins/nuclei-templates/ -c $THREADS -target http://$TARGET -o $LOOT_DIR/web/nuclei-http-${TARGET}-port80.txt
		nuclei -silent -t /usr/share/Maxl0x/plugins/nuclei-templates/ -c $THREADS -target https://$TARGET -o $LOOT_DIR/web/nuclei-https-${TARGET}-port443.txt 
	fi
