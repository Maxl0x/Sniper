...
if [[ "$MODE" = "flyover" ]]; then
  ...
  if [[ "$REPORT" = "1" ]]; then
    ...
    echo -e "$OKRED "
    echo -e "$RESET"
    echo "Maxl0x -f $FILE -m $MODE --noreport $args" >> $LOOT_DIR/scans/$WORKSPACE-$MODE.txt
    Maxl0x $args | tee $WORKSPACE_DIR/output/Maxl0x-$WORKSPACE-$MODE-`date +"%Y%m%d%H%M"`.txt 2>&1
    echo "$FILE $MODE `date +"%Y-%m-%d %H:%M"`" 2> /dev/null >> $LOOT_DIR/scans/tasks.txt 2> /dev/null
    echo "Maxl0x -f $FILE -m $MODE --noreport $args" >> $LOOT_DIR/scans/running_${WORKSPACE}_${MODE}.txt
    ...
    for HOST in `cat $FILE`; do
      ...
      echo "Maxl0x -t $TARGET -m $MODE $args" >> $LOOT_DIR/scans/running_${TARGET}_${MODE}.txt 2> /dev/null
      ls -lh $LOOT_DIR/scans/running_*.txt 2> /dev/null | wc -l 2> /dev/null > $LOOT_DIR/scans/tasks-running.txt
      echo -e "${OKGREEN}====================================================================================${RESET}•x${OKGREEN}[`date +"%Y-%m-%d](%H:%M)"`${RESET}x•"
