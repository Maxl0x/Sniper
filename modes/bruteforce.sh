if [[ "$AUTO_BRUTE" = "1" ]]; then
  echo "Maxl0x -t $TARGET -m $MODE --noreport $args" >> $LOOT_DIR/scans/running_${TARGET}_bruteforce.txt 2> /dev/null
  ...
  echo -e "$OKORANGE + -- --=[ AUTO_BRUTE setting disabled in Maxl0x.conf... skipping.$RESET"
fi