...
if [[ "$MODE" = "fullportonly" ]]; then
  if [[ "$REPORT" = "1" ]]; then
    ...
    echo "Maxl0x -t $TARGET -m $MODE --noreport " >> $LOOT_DIR/scans/running_${TARGET}_${MODE}.txt
  ...
  if [[ -f "/usr/share/Maxl0x/pro/.portscanner.conf" ]]; then
    source /usr/share/Maxl0x/pro/.portscanner.conf
  fi
  if [[ -z "$PORT" ]]; then
